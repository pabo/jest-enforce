...
const jestIsGlobal = () => {
  return exec('jest', ['--version']).then(result => !!result).catch(error => false);
};
...
function matchesAnExpression(input, regExpressions) {
    ...
    testFiles.reduce((promiseChain, filePath, index) => {
      return promiseChain.then(() => {
        return new Promise((resolve, reject) => {
          ...
          exec('jest', [filePath, '--config=' + JSON.stringify(jestConfig), '--coverage']).then(() => {
            let data;
...
async function _getApplicableTests(testScope) {
  return Promise.all([jestIsGlobal(), jestIsModule()]).then(([isGlobal, isModule]) => {
...
function _getExtraneous({ filePath, report }) {
  ...
  const extraneousFiles = report.reduce((prev, currentFile) => {
    ...
    if (!isWhitelisted) {
      const hasCoverage = parseInt(currentFile.metrics.coveredstatements) || parseInt(currentFile.metrics.coveredconditionals) || parseInt(currentFile.metrics.coveredmethods);
      if (hasCoverage) {
